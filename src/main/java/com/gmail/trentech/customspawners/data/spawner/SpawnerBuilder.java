...
import static com.gmail.trentech.customspawners.data.DataQueries.ENTITY;
...
public class SpawnerBuilder extends AbstractDataBuilder<Spawner> {
    ...
    protected Optional<Spawner> buildContent(DataView container) throws InvalidDataException {
        if (container.contains(ENTITY, LOCATION, AMOUNT, TIME, RANGE, ENABLE)) {
        	String entity = container.getString(ENTITY).get();
            ...
            return Optional.of(new Spawner(entity, location, amount, time, range, enable));
        }
